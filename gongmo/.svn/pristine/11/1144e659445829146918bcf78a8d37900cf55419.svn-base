	 		...
	 		if(adminType != '1'){
	 			alert('권한이 없습니다.');
	 				...
	 				if(confirm("정말 삭제하시겠습니까?") == true){
	 				 $.ajax({
	 						url: "/gongmo/notice/noticeDeleted.action",
	 						type:"post",
	 						data:{ noticeNo : $(this).attr('data-noticeNo') },
	            ...
	            <div class="panel panel-default panel-table">
	              <div class="panel-heading">
	                <div class="row">
	                  ...
	                  <c:choose>
	                  	<c:when test="${ loginuser.adminType eq  '1' }">
		                  <div class="col col-xs-6 text-right">
		                    <button type="button" class="btn btn-sm btn-primary btn-create" 
		                    		style="background-color: #d75050; border-color: #d75050"
		                    		onclick="location.href='noticeregister.action'">글쓰기</button>
		                  </div>
		                 </c:when>
	                  </c:choose>
	                </div>
	              ...
	              <div class="panel-body">
	                <table class="table table-striped table-bordered table-list">
	                  ...
	                  <tbody>
							...
							<c:choose>
							<c:when test="${notice.sellStatue eq 'true' }">
							</c:when>
							<c:otherwise>
							<tr>
								<td align="center">
		                            <a class="btn btn-danger" id="noticeDeleted" data-noticeNo="${notice.noticeNo}"><em class="fa fa-trash"></em></a>
		                        	<input type="hidden" id="loginuser" value="${loginuser.adminType}"/>
		                        </td>
		                        <td class="hidden-xs">${ notice.noticeNo }</td>
								<td>						
									<a href='noticedetail.action?noticeNo=${ notice.noticeNo }'>
									${ notice.title }
									</a>
								</td>
								<td>${ notice.writer }</td>
