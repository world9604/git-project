...
<script type="text/javascript"
	src="//apis.daum.net/maps/maps3.js?apikey=09f9640d9f08c86ae0312c049c36bbbf&libraries=services"></script>
<style> 
...
<script type="text/javascript">
	...
	$(function() {
		$('#interest').click(function(event) {
			if (confirm("관심 매물로 등록하시겠습니까?") == true) {
				$.ajax({
					url : "/gongmo/member/interestStore.action",
					type : "post",
					data : {
						memberNos : $('#memberNo').val(),
						storeNos : $('#storeNo').val()
					},
					success : function(data, status, xhr) {
						console.log(data);
						if (data == "success") {
							alert('관신 매물에 등록되었습니다..');
							location.reload();
						} else {
							alert('이미 관신 매물에 등록된 매물입니다.');
						}
					}
				});
			}
	...
	$(function() {
		$('#question').click(function(event) {
			...
			if (!questionTitle) {
				alert('제목을 입력해주세요');
				return;
			} else if (!questionContents) {
				alert('내용을 입력해주세요');
				return;
			} else {

				var question = $("#questionForm").serialize();

				$.ajax({
					url : "/gongmo/stores/question.action",
					type : "POST",
					data : question,
					success : function(data, status, xhr) {

						if (data == "success") {
							alert('전송이 정상적으로 처리되었습니다.');
						} else {
							alert('전송이 비정상적으로 처리되었습니다.');
						}
					}
				});
...
<body style="padding-right: inherit;">

	<c:import url="/WEB-INF/views/include/header.jsp" />
	<!-- ///////////////////////////carousel(부트스트랩)////////////////////// -->
	<!-- style="z-index:-9999;"(화면 최상위에 올리는 구문) id="carousel-example-generic" class="carousel slide" data-ride="carousel" data-interval="3000"(화면이 넘어가는 시간을 선택) -->
	...
	<!-- ///////////////////////////carousel(부트스트랩) 끝!!!!!!////////////////////// -->
	<!-- /////////////////////////// affix(메뉴)부트스트랩 ///data-offset-top="60"  data-offset-bottom="200" 쪽지함기능//////////////////// -->
	<!-- ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// -->
	<div class="container" id="store">
		<div class="row">
			...
			<div class="col-md-2">
				<c:if test="${interestStore.memberNo eq null}">
					<h3>
						<input class="btn" type="submit" id="interest"
							style="width: 100%; height: 100%; background-color: aliceblue;"
							value="관심매물 등록" />
					</h3>
				...
				<c:if test="${interestStore.memberNo == loginuser.memberNo}">
				</c:if>
			</div>
				</c:when>
			</c:choose>
		...
		<div class="row">
			...
			<div class="col-md-2">
				...
				<c:choose>
					<c:when test="${not empty sessionScope.loginuser}">
					<input class="btn" type="submit" data-toggle="modal"
						data-dismiss="modal" data-target="#myModal0"
						style="width: 100%; height: 100%; background-color: aliceblue;"
						value="문의 보내기" />
					</c:when>
				</c:choose>
	 	...
	 	<div class="row" style="padding-top: 5px;">
			<div class="col-md-offset-1">
				<c:forEach var="storeImage" items="${storeImages}"
					...
					<div style="display: inline;" data-toggle="modal"
						data-target="#myModal11">
						<img class="imageList" style="width: 200px; height: 200px;"
		...
		<div class="buttons" style="text-align: center"> 
			<a href='allStroreList.action' style="font-size: 30px">목록보기</a>
		</div>
		
		<br /><br />		
			
		<div id="firstmap">
		 	<div data-toggle="tooltip" data-placement="top" title="지도를 클릭 해보세요">
		 	</div>
	...
	<!-- /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// -->
	<div class="modal fade" id="myModal0" aria-labelledby="myModalLabel"
		style="z-index: 9999;">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">문의 하기</h4>
				</div>
				<div class="modal-body">
					<form id="questionForm" class="form form-signup">
						<div class="form-group">
							<div class="input-group">
								<span class="input-group-addon"> <!-- 아이콘 --> <span
									class="glyphicon glyphicon-envelope"></span></span>
								<h5>${store.storeName}</h5>
								<input type="hidden" id="lonmemberNo" name=lonmemberNo
									value="${loginuser.memberNo}"> <input type="hidden"
									id="memberNo" name="memberNo" value="${loginuser.memberNo}">
								<input type="hidden" id="storeNo" name="storeNo"
									value="${store.storeNo}">
									<input type="hidden" id="storeNo" name="storeMemberNo"
									value="${store.memberNo}"> 
							</div>
						</div>
						<div class="form-group">
							<div class="input-group">
						...
						<div class="form-group">
							<div class="input-group">
								...
								<p class="help-block">문의 내용</p>

								<textarea id="questionContents" name="questionContents" rows="5"
									cols="70%"></textarea>
							</div>
						</div>
						<div class="modal-footer">
							...
							<button class="btn btn-warning" id="question">
...
<script src="//d3js.org/d3.v3.min.js"></script><!-- d3.js로 json데이터 파일 읽기 -->
<script>
...
mapOption = {
    center: new daum.maps.LatLng(storePointJson.lat, storePointJson.lng), // 지도의 중심좌표
    level: 3 // 지도의 확대 레벨
};  
...
var map = new daum.maps.Map(mapContainer, mapOption); 
 ...
 //마커가 표시될 위치입니다 
var markerPosition  = new daum.maps.LatLng(storePointJson.lat, storePointJson.lng); 
...
daum.maps.event.addListener(map, 'click', function(mouseEvent) {        
		 ...
		 // 커스텀 오버레이를 생성하고 지도에 표시한다
			customOverlay = new daum.maps.CustomOverlay({
				...
				content: content, 
				position: new daum.maps.LatLng(latlng.getLat(), latlng.getLng()), // 커스텀 오버레이를 표시할 좌표
				xAnchor: 0.5, // 컨텐츠의 x 위치
				yAnchor: 0.4 // 컨텐츠의 y 위치
			});		    
		    
		  
			$('.overlaybox #radiusBtn').on('click',function(){
				
				//화면을 가리고 로딩 이미지를 띄어줌(10초간)
				wrapWindowByMask();
				setTimeout(function() {
	    			closeWindowByMask();
	    		   }, 10000);
				...
				var radius = $('.overlaybox #radius option:selected').val();//100m 등등
				var subRadius = radius.substring(0,3);
				customOverlay.setMap(null); // 커스텀 오버레이를 지도에서 제거한다.
				
				$('#firstmap').empty();
				 $('#firstmap').append("<div class='container'>"+
	      				"<div class='row'>"+
  ...
  "</div><!--<div class='col-xs-18 col-sm-6 col-md-5' style='padding-top:10px'> END-->"+
    ...
    "</div><!-- row END -->"+
			  ...
			  "<br><br>");  
				...
				// 지도를 생성합니다    
				map0 = new daum.maps.Map(mapContainer0, mapOption0); 
				 //마커가 표시될 위치입니다 
				var markerPosition  = new daum.maps.LatLng(storePointJson.lat, storePointJson.lng); 
				// 마커를 생성합니다
				var marker = new daum.maps.Marker({
				...
				marker.setMap(map0); //해당 가게 마커
			     ...
			     var circle = new daum.maps.Circle({
			        center : new daum.maps.LatLng(storePointJson.lat, storePointJson.lng),  // 원의 중심좌표 입니다 
			    ...
			    $.ajax({
	    			...
	    			data: {storeLat : storePointJson.lat, storeLng : storePointJson.lng, radius: subRadius, storeNo: storeNo},
	    			success:function(data, status, xhr){
	    				
	    				var arroundStore = data.split('|');
	    				 ...
	    				 console.log(arroundPark)
	    				 console.log(arroundSubway)
	    				 console.log(arroundSchool)
	    				 console.log(subwayItems)
	    				 console.log(dateSubwayItems)
	    				 console.log(busStationItems)
	    				 console.log(aroundStoreItems);
	    				 // 주차장 마커가 표시될 좌표 배열입니다
	    				 for(var i=0; i<arroundPark.length; i++){
	    				 	carparkPositions.push(new daum.maps.LatLng(arroundPark[i].latitude, arroundPark[i].longitude));
	    				 }
					
	    				 // 지하철 마커가 표시될 좌표 배열입니다
	    				 for(var i=0; i<arroundSubway.length; i++){
	    				 	subwayPositions.push(new daum.maps.LatLng(arroundSubway[i].latitude, arroundSubway[i].longitude));
	    				 }

	    				 //학교 마커가 표시될 좌표 배열입니다
	    				 for(var i=0; i<arroundSchool.length; i++){
	    				 	schoolPositions.push(new daum.maps.LatLng(arroundSchool[i].latitude, arroundSchool[i].longitude));
	    				 }
...
///////////////////////////////////////밑에 구글 차트///////////////////////////////////////////
    				console.log(subwayItems);
 	     			google.charts.load('current', {'packages':['bar', 'line', 'geochart', 'corechart']});
...
////////////////////////////////////유사업종List/////////////////////////////////////////////////////
				  ...
				  $.each(aroundStoreItems, function(index, aroundStoreItem){
	    			    $('.filterable tbody').append(
	   	    				"<tr>"+                       
	   	                    "<td>"+aroundStoreItem.bizesNm+"</td>"+
	   	                    "<td>"+aroundStoreItem.indsSclsNm+"</td>"+
	   	                    "<td>"+aroundStoreItem.lnoAdr+"</td>"+
	   	                    "<td>"+aroundStoreItem.flrNo+"</td>"+
	   	                    "<td>"+aroundStoreItem.telno+"</td>"+
	   	                	"</tr>");
	    		   }); 
...
function createStoreMarkers() {
    for (var i = 0; i < subwayPositions.length; i++) {
        ...
        // 마커이미지와 마커를 생성합니다
        var markerImage = createMarkerImage(markerImageSrc, imageSize, imageOptions),    
            marker = createMarker(subwayPositions[i], markerImage);  
...
function getSubwayItem(){
	...
	if(subwayItems.length != 0){
		...
		var dataSubwayItem = [];
		dataSubwayItem.push('해당하는 지하철역이 없습니다');
		dataSubwayItem.push(0);
		dataSubwayItem.push(0);
		dataSubwayItems.push(dataSubwayItem);//2차원배열
	}
...
function getSubwayItemByDate(){
	  ...
	  if(dateSubwayItems != null){
		  
		  for(var i=0; i<days.length; i++){
				
			  if(dateSubwayItems[i] != null){
			  
			  var dataSubwayItem2 = [];
					dataSubwayItem2.push(days[i]);//지하철 역명 대신 요일이 들어가야 한다
					dataSubwayItem2.push(parseInt(dateSubwayItems[i].RIDE_PASGR_NUM));
					dataSubwayItem2.push(parseInt(dateSubwayItems[i].ALIGHT_PASGR_NUM));
					dataSubwayItems2.push(dataSubwayItem2);//2차원배열
			  }else{
				  var dataSubwayItem2 = [];
					dataSubwayItem2.push(days[i]);//지하철 역명 대신 요일이 들어가야 한다
					dataSubwayItem2.push(0);
					dataSubwayItem2.push(0);
					dataSubwayItems2.push(dataSubwayItem2);//2차원배열
			  }
			}	    				 
	  ...
	  if(dateSubwayItems != null){
		  ...
		  data2.addColumn('string', "해당하는 역이 없습니다");	
	  }
...
function getButStaionInfo(){
	   ...
	   if(busStationItems.length != 0){	   
			for(var i=0; i<busStationItems.length; i++){
				
			
				if(busStationItems[i].RIDE_PASGR_NUM != null && busStationItems[i].RIDE_PASGR_NUM != ''){
					var dataBusStationItem = [];
					dataBusStationItem.push(busStationItems[i].BUS_STA_NM);
		   ...
		   var dataBusStationItem = [];
			dataBusStationItem.push("해당하는 버스 정류장이 없습니다");
			dataBusStationItem.push(0);
			dataBusStationItem.push(0);
	  ...
	  var data3 = google.visualization.arrayToDataTable(dataBusStationItems);
...
function getGeoChart(){
	  ...
	  var data5 = new google.visualization.DataTable();
	  	...
	  	data5.addColumn({type:'string', role:'tooltip'});
	  	
	  	var ivalue = new Array();
	  	console.log(wholeCountrySales);
	  	 
	  	for(var i=0; i<wholeCountrySales.length; i++){
	  		data5.addRows([[{v:wholeCountrySales[i].v, f:wholeCountrySales[i].region+'   월평균매출액(만원)'},i,'관광/여가/오락:'+wholeCountrySales[i].hobbyMonth+'  생활서비스:'+wholeCountrySales[i].serviceMonth+' 소매:'+wholeCountrySales[i].retailMonth+'  숙박:'+wholeCountrySales[i].accomodationMonth+'  스포츠:'+wholeCountrySales[i].sportsMonth+'  음식:'+wholeCountrySales[i].foodMonth+'  의료:'+wholeCountrySales[i].medicalMonth+'  학문/교육:'+wholeCountrySales[i].educationMonth]]);
	  	...
	  	var chart5 = new google.visualization.GeoChart(document.getElementById('visualization')); 
	  	google.visualization.events.addListener(chart5, 'select', function() {
	  	 	var selection = chart5.getSelection();
	  	 	if (selection.length == 1) {
	  	 		var selectedRow = selection[0].row;
	  	 		var selectedRegion = data5.getValue(selectedRow, 0);
	  	 		 if(ivalue[selectedRegion] != '') {
	  	 			//document.getElementsByTagName('body')[0].style.background=ivalue[selectedRegion]; 
	  	 			
	  	 			$.each(wholeCountrySales, function(index, wholeCountrySale){
	  	 				if(wholeCountrySale.v == selectedRegion){
	  	 					
	  	 					var data=[
                             ['업종', '월매출', '건단가'],
                             ['관광/여가/오락', parseInt(wholeCountrySale.hobbyMonth.replace(/,/g,"")), parseInt(wholeCountrySale.hobbyUnitPrice.replace(/,/g,""))],
                             ['생활서비스', parseInt(wholeCountrySale.serviceMonth.replace(/,/g,"")), parseInt(wholeCountrySale.serviceUnitPrice.replace(/,/g,""))],
                             ['소매', parseInt(wholeCountrySale.retailMonth.replace(/,/g,"")), parseInt(wholeCountrySale.retailUnitPrice.replace(/,/g,""))],
                             ['숙박', parseInt(wholeCountrySale.accomodationMonth.replace(/,/g,"")), parseInt(wholeCountrySale.accomodationUnitPrice.replace(/,/g,""))],
                             ['스포츠', parseInt(wholeCountrySale.sportsMonth.replace(/,/g,"")), parseInt(wholeCountrySale.sportsUnitPrice.replace(/,/g,""))],
                             ['음식', parseInt(wholeCountrySale.foodMonth.replace(/,/g,"")), parseInt(wholeCountrySale.foodUnitPrice.replace(/,/g,""))],
                             ['의료', parseInt(wholeCountrySale.medicalMonth.replace(/,/g,"")), parseInt(wholeCountrySale.medicalUnitPrice.replace(/,/g,""))],
                             ['학문/교육', parseInt(wholeCountrySale.educationMonth.replace(/,/g,"")), parseInt(wholeCountrySale.educationUnitPrice.replace(/,/g,""))]
                            ];
...
//////////////////////////////지도2/////////////////////////////////////////////////////////
	...
	if(aroundStoreItems.length != 0){
		for(var i=0; i<aroundStoreItems.length; i++){
			
			var colorCode= "#"+Math.round(Math.random()*0xFFFFFF).toString(16);//색상 코드 랜덤
			
				// 지도에 표시할 원을 생성합니다
				var circle2 = new daum.maps.Circle({
				   center : new daum.maps.LatLng(aroundStoreItems[i].lat, aroundStoreItems[i].lon),  // 원의 중심좌표 입니다 
				...
				var iwContent = '<div style="padding:5px;">'+aroundStoreItems[i].bizesNm+'</div>', 
			    iwPosition = new daum.maps.LatLng(aroundStoreItems[i].lat, aroundStoreItems[i].lon), //인포윈도우 표시 위치입니다
...
function getUpjongChart(){
	                                                    ...
	                                                    chart6 = new google.visualization.ComboChart(document.getElementById('dual_x_div'));
	                                                    current = 0;
	                                                    ...
	                                                    upjongChart=function drawGetUpjongChart(wholeCountrySale) {
	                                                    	...
	                                                    	var data6 = new google.visualization.arrayToDataTable(wholeCountrySale);
